...
describe('NftCollection', () => {
    ...
    const CONTENT_URL =
        'https://violet-traditional-rabbit-103.mypinata.cloud/ipfs/QmRh1kHRC9FdVQvnvZVDKanXYWaSpdYoNC5pUkBB4s5TW2/'; // Replace with your actual content URL
        ...
        const contentCell = beginCell().storeUint(OFFCHAIN_CONTENT_PREFIX, 8).storeStringRefTail(CONTENT_URL).endCell();
         ...
         nftCollection = blockchain.openContract(
            await NftCollection.fromInit(
                ...
                contentCell,
                NFT_PRICE,
                false, // Enable whitelist by default
            ),
        );
    ...
    it('should successfully transfer NFT to new owner', async () => {
        // First mint an NFT
        const nftMintTotalCost = await nftCollection.getGetNftMintTotalCost();
        await nftCollection.send(
            owner.getSender(),
        ...
        // Create a new treasury contract to act as the new owner
        const newOwner = await blockchain.treasury('newOwner');

        // Open the NFT contract
        ...
        const transferResult = await nftItem.send(
            owner.getSender(),
            {
                value: toNano('0.1'), // Enough value to cover gas
            },
            {
                $$type: 'Transfer',
                query_id: 0n,
        ...
        // Check if the transfer transaction was successful
        ...
        expect(nftData.owner_address.toString()).toEqual(newOwner.address.toString());

        // Try to transfer again from the old owner (should fail)
        const failedTransferResult = await nftItem.send(
            owner.getSender(),
            {
                value: toNano('0.1'),
            },
        ...
        // This should fail as the old owner no longer owns the NFT
        expect(failedTransferResult.transactions).toHaveTransaction({
            from: owner.address,
            to: nftAddress,
            success: false,
        });

        // Verify the new owner can transfer the NFT
        const newTransferResult = await nftItem.send(
            newOwner.getSender(),
            {
                value: toNano('0.1'),
            },
            {
                $$type: 'Transfer',
                query_id: 0n,
                new_owner: owner.address, // Transfer back to original owner
                response_destination: null,
                custom_payload: null,
                forward_amount: 0n,
                forward_payload: beginCell().endCell().beginParse(),
        ...
        // This should succeed as the new owner is making the transfer
